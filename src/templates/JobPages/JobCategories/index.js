              ...
              <div className="card jobs-categories">
                <div className="card-body">
                  {(jobCategories1 || []).map((item, key) => (
                    <Link key={key} to="#!" className="px-3 py-2 rounded bg-light bg-opacity-50 d-block mb-2">
                      {item.name}
                      <span className="badge text-bg-info float-end bg-opacity-100">{item.category}</span>
                    </Link>
                  ))}
                </div>
              </div>
