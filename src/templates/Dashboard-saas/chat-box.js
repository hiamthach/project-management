          ...
          <CardBody className="pb-0">
            <div>
              <div className="chat-conversation">
                <PerfectScrollbar style={{ marginBottom: '1rem', maxHeight: '260px' }} containerRef={(ref) => setMessageBox(ref)}>
                  <ul className="list-unstyled">
                    ...
                    {(messages || []).map((message, index) => (
                      <React.Fragment key={index}>{message['isSender'] ? <Sender message={message} /> : <Reciver message={message} />}</React.Fragment>
                    ))}
                  </ul>
                </PerfectScrollbar>
              </div>
